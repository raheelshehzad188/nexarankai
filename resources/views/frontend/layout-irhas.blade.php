@php
    ...
    if (isset($blogPost)) {
        $seoMetaTitle = $blogPost->meta_title ?? $blogPost->title;
        $seoMetaDesc = $blogPost->meta_description ?? ($blogPost->excerpt ?? $siteName);
        $seoKeywords = $blogPost->meta_keywords ?? null;
        $seoCanonical = $blogPost->canonical_url ?: url()->current();
        $seoOgTitle = $blogPost->og_title ?: ($seoMetaTitle . ' - ' . $siteName);
        $seoOgDesc = $blogPost->og_description ?: $seoMetaDesc;
        $seoOgImage = $blogPost->og_image ? $blogPost->imageUrl('og_image') : $blogPost->imageUrl('featured_image');
    } elseif (isset($service)) {
        $seoMetaTitle = $service->meta_title ?? $service->title;
        $seoMetaDesc = $service->meta_description ?? ($service->excerpt ?? $siteName);
        $seoKeywords = $service->meta_keywords ?? null;
        $seoCanonical = $service->canonical_url ?: url()->current();
        $seoOgTitle = $service->og_title ?: ($seoMetaTitle . ' - ' . $siteName);
        $seoOgDesc = $service->og_description ?: $seoMetaDesc;
        $seoOgImage = $service->og_image ? $service->imageUrl('og_image') : $service->imageUrl('image');
    } else {
    ...
    $bodyClass = $bodyClass ?? 'irhas3 home3';
@endphp
<!DOCTYPE html>
...
<body class="{{ $bodyClass }}">
    <div id="main-wrapper" class="animsition clearfix">
        @include('frontend.partials.irhas.header')

        <section class="content">
            @yield('content')
        </section>

        @include('frontend.partials.irhas.footer')
    </div>
