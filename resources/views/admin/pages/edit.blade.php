...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.pages.update', $page) }}" method="POST">
            ...
            @php
                $currentLayout = old('layout', (
                    ($page->use_irhas2_layout ?? false) ? 'irhas2' :
                    (($page->use_irhas_layout ?? false) ? 'irhas' :
                    (($page->use_new_layout ?? false) ? 'new' : 'default'))
                ));
            @endphp
            <div class="mb-3">
                ...
                <select class="form-select @error('layout') is-invalid @enderror" id="layout" name="layout" required>
                    <option value="default" {{ $currentLayout === 'default' ? 'selected' : '' }}>Default Layout</option>
                    <option value="irhas" {{ $currentLayout === 'irhas' ? 'selected' : '' }}>Irhas Layout (Home 3)</option>
                    <option value="irhas2" {{ $currentLayout === 'irhas2' ? 'selected' : '' }}>Irhas Layout (Home 2)</option>
                    <option value="new" {{ $currentLayout === 'new' ? 'selected' : '' }}>New Layout</option>
                </select>
